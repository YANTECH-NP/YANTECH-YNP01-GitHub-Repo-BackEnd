...
# =============================================================================
# YANTECH SNS Email Test Script
# Tests email notifications via SNS topic
# =============================================================================
...
response=$(curl -s -w "\n%{http_code}" -X POST "$CLIENT_API_URL/notifications" \
    ...
    -d '{
        ...
        "Subject": "Email Test from YANTECH Platform",
        "Message": "This is a dedicated email test from your YANTECH notification system.",
        "OutputType": "EMAIL",
...
if [ "$http_code" = "200" ]; then
    ...
    # Quick check for email processing (optimized)
    ...
    for i in {1..3}; do
        ...
        # Get recent entries and check for our email
        ...
        RECENT_DELIVERY=$(echo "$RECENT_ENTRIES" | jq -r ".Items[] | select(.Request.S | contains(\"bemnjichiella@gmail.com\") and contains(\"Email Test from YANTECH Platform\")) | select(.Timestamp.S > \"$CUTOFF_TIME\") | .Status.S" | head -1)
        
        if [ "$RECENT_DELIVERY" = "delivered" ]; then
            DELIVERY_TIME=$(echo "$RECENT_ENTRIES" | jq -r ".Items[] | select(.Request.S | contains(\"bemnjichiella@gmail.com\") and contains(\"Email Test from YANTECH Platform\")) | select(.Timestamp.S > \"$CUTOFF_TIME\") | .Timestamp.S" | head -1)
            ...
            DELIVERY_ERROR=$(echo "$RECENT_ENTRIES" | jq -r ".Items[] | select(.Request.S | contains(\"bemnjichiella@gmail.com\") and contains(\"Email Test from YANTECH Platform\")) | select(.Timestamp.S > \"$CUTOFF_TIME\") | .Error.S" | head -1)
